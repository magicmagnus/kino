...
const MovieSelectionButton = (props) => {
    ...
    return (
        <button
            onClick={onClick}
            className={
                "group flex h-24 min-w-56 rounded-lg text-xs font-semibold transition-all duration-200 hover:bg-rose-500 hover:text-rose-50 sm:min-w-56" +
                (selected
        ...
        >
            ...
            <div className="flex h-full w-full flex-col items-start justify-center gap-1.5 overflow-hidden text-wrap px-1.5 pl-3 text-left">
                <p className="line-clamp-4 break-words">{text}</p>
            </div>
        </button>
    );
};
